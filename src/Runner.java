...
public class Runner {
	...
	public static void main(String[] args) {
		...
		System.out.println("saveToManyPDF - zapisanie wielu dokumentów w PDF");
...
/*
	...
	private static void saveTo1PDF(ArrayList <Dokument> lista) throws IOException {
		...
		document.open();
			for (int i=0; i<lista.size(); i++) {
				document.add(new Paragraph("saveTo1PDF"));
				document.add(lista.get(i).getTytul()+"\n");
				document.add(lista.get(i).getTresc()+"\n");
				document.close(); 
				System.out.println("PDF saved");
			}
			  try{
...
/*		
private static void saveToManyPDF(ArrayList <Dokument> lista) throws IOException {
		...
		try{
			...
			for (int i=0; i<lista.size(); i++) {
				...
				document.open();
					document.add(new Paragraph("saveToManyPDF"+i));
					document.close(); 
				}
			}catch(FileNotFoundException e){
			e.printStackTrace();
